...
from src.datasets.annotations import raw_predictions_to_actions, prepare_game_spotting_results
from src.utils import get_best_model_path, get_video_info
from src.evaluation.predictor import MultiDimStackerPredictor
from src.data_processing.frame_fetchers.nvdec import NvDecFrameFetcher
...
def get_raw_predictions(predictor: MultiDimStackerPredictor,
    ...
    with tqdm() as t:
        while True:
            ...
            t.update()
            if predict_index > max_frame_index or torch.all(frame==0) :
                break
    predictor.reset_buffers()
...
def predict_video(predictor: MultiDimStackerPredictor,
                  ...
                  constants,
                  single_match=False) -> dict[str, tuple]:
    print(game_dir)
    video_path = game_dir if single_match else game_dir / f"{half}_{RESOLUTION}.mkv"
...
def predict_game(predictor: MultiDimStackerPredictor,
    ...
    for half in constants.halves:
        class_actions = predict_video(
            predictor, half, game_dir, game_prediction_dir, use_saved_predictions, constants, single_match
        )
...
def predict_split(experiment: str, split: str, gpu_id: int, use_saved_predictions: bool, constants):
    ...
    for game in games:
        predict_game(predictor, game, prediction_dir, use_saved_predictions, constants)

def predict_single_video(experiment: str, split: str, gpu_id: int, use_saved_predictions: bool, prediction_dir: str, constants, single_match: bool=False):
    experiment_dir = constants.experiments_dir / experiment
    model_path = get_best_model_path(experiment_dir)
    print("Model path:", model_path)
    predictor = MultiDimStackerPredictor(model_path, device=f"cuda:{gpu_id}", tta=TTA)
    game = constants.test_games[0]
    if not prediction_dir.exists():
        prediction_dir.mkdir(parents=True, exist_ok=True)
    else:
        print(f"Folder {prediction_dir} already exists.")
    predict_game(predictor, game, prediction_dir, use_saved_predictions, constants, single_match=single_match)


if __name__ == "__main__":
    args = parse_arguments()
    from src import constants as const
    predict_split(args.experiment, args.split, args.gpu_id, args.use_saved_predictions, const)